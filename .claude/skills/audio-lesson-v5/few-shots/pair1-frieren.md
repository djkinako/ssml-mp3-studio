...
| 2 | **凱旋** | 難読「凱」、文の意味のコア、日本語と同形だが書面語専用 |
...
- **4ブロックフル**: ベース → 漢字書取 → 語彙深掘り → 文法、全てを使って深掘りする典型例
...
前半の「われ」は日本語の「我々」の「われ」と同じ、自分を指す字や。後半の「メン」がポイントで、これ日本語にはない繁体字、ニンベンに「門」を組み合わせる。複数形を作る接尾辞の字やで。<break time="1.5s"/>

ふたつ目、<lang xml:lang="cmn-TW">凱旋</lang>(カイセン、凱旋)。<break time="500ms"/>
「カイ」が難読ポイント、書き方は左側が<sub alias="アニ">豈</sub>、古い書面語に出る字、右側が机の脚みたいな<sub alias="キ">几</sub>を組み合わせる。「セン」は「旋回」の「せん」、方偏に<sub alias="ヒキ">疋</sub>を書いた形やな。日本語と同じ熟語やから、組み合わせで覚えられるで。<break time="1.5s"/>
...
これも一文字単語の例外枠や。日本語の「帰る」の繁体字版で、ごっつい字。左側に「自ら」のような形と「止まる」を縦に重ね、右側に「ほうき」の右半分みたいな形を書く。意味は同じく「帰る」、書面語向きの言い回しに使うで。<break time="2s"/>
...
- 鬼門ワード(「画」「字義」「然り而して」「豈」「几」「疋」): `<sub alias>` 内のみで地の文に裸出しなし ✅
- 単独漢字裸出し: 「我」「歸」など全て熟語経由 or `<lang>` タグ内 or `<sub alias>` で対応 ✅
- 二度読み(漢字+カッコふりがな): 「我們」(ウォーメン、私たち)はカタカナ読み+意味の二重情報なので二度読みではない ✅
